...
class SMPLNodes(RigidNodes):
    ...
    def get_gaussians(self, cam: dataclass_camera) -> Dict[str, torch.Tensor]:
        ...
        # check nan/inf in gs_dict
        # During training we keep strict checks to fail fast.
        # During evaluation (e.g. novel-view rendering), sanitize values to avoid
        # crashing the whole scene-level pipeline at the very end.
        for k, v in gs_dict.items():
            has_nan = torch.isnan(v).any()
            has_inf = torch.isinf(v).any()
            if has_nan or has_inf:
                ...
                logger.warning(
                    f"Detected invalid values in gaussian {k} at step {self.step} during eval; applying nan_to_num fallback."
                )
                gs_dict[k] = torch.nan_to_num(v, nan=0.0, posinf=1e4, neginf=-1e4)
