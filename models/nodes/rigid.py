...
class RigidNodes(VanillaGaussians):
    ...
    def refinement_after(self, step: int, optimizer: torch.optim.Optimizer) -> None:
        ...
        with torch.no_grad():
            ...
            if do_densification:
                ...
                high_grads = (avg_grad_norm > self.ctrl_cfg.densify_grad_thresh).reshape(-1)
                ...
                if self.step < self.ctrl_cfg.stop_screen_size_at:
                    splits |= (self.max_2Dsize > self.ctrl_cfg.split_screen_size).reshape(-1)
                splits &= high_grads
    ...
    def cull_gaussians(self):
        ...
        # Keep mask 1D even when num_points == 1.
        culls = (self.get_opacity.data < self.ctrl_cfg.cull_alpha_thresh).reshape(-1)
        ...
        if self.step > self.ctrl_cfg.reset_alpha_interval:
            ...
            if self.step < self.ctrl_cfg.stop_screen_size_at:
                ...
                culls = culls | (self.max_2Dsize > self.ctrl_cfg.cull_screen_size).reshape(-1)
    ...
    def split_gaussians(self, split_mask: torch.Tensor, samps: int = 2) -> Tuple:
        ...
        """
        split_mask = split_mask.reshape(-1).bool()

        n_splits = split_mask.sum().item()
        print(f"    Split: {n_splits}")

        if n_splits == 0:
            empty_means = self._means.new_empty((0, self._means.shape[-1]))
            empty_feature_dc = self._features_dc.new_empty((0, self._features_dc.shape[-1]))
            empty_feature_rest = self._features_rest.new_empty((0,) + tuple(self._features_rest.shape[1:]))
            empty_opacities = self._opacities.new_empty((0, self._opacities.shape[-1]))
            empty_scales = self._scales.new_empty((0, self._scales.shape[-1]))
            empty_quats = self._quats.new_empty((0, self._quats.shape[-1]))
            empty_ids = self.point_ids.new_empty((0, self.point_ids.shape[-1]))
            return (
                empty_means,
                empty_feature_dc,
                empty_feature_rest,
    ...
    def dup_gaussians(self, dup_mask: torch.Tensor) -> Tuple:
        ...
        """
        dup_mask = dup_mask.reshape(-1).bool()
    ...
    def compute_reg_loss(self) -> Dict[str, torch.Tensor]:
        ...
        if scaling_reg is not None:
            ...
            if self.step < stop_after and self.step > start_after and w > 0:
                gs_scales = self._gs_cache["_scales"]
                if gs_scales.numel() > 0 and gs_scales.shape[0] > 0:
                    scale_prod = gs_scales.prod(dim=-1)
                    kth = max(1, int(scale_prod.shape[0] * precentile))
                    p = torch.kthvalue(scale_prod, kth).values
