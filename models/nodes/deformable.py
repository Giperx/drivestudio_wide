...
class DeformableNodes(RigidNodes):
    ...
    def get_deformation(self, local_means) -> Tuple:
        ...
        """
        # Guard against accidental shape drift (e.g. singleton cull producing extra dims).
        point_ids = self.point_ids.reshape(-1, self.point_ids.shape[-1])[:, 0]
        local_means = local_means.reshape(-1, local_means.shape[-1])

        # No points left for this class.
        if local_means.shape[0] == 0:
            return None, None, None

        assert local_means.shape[0] == point_ids.shape[0], \
            "its a bug here, we need to pass the mask for points_ids"

        nonrigid_embed = self.instances_embedding[point_ids].reshape(local_means.shape[0], -1)
        ins_height = self.instances_size[point_ids][:, 2].clamp_min(1e-6)
        x = local_means.data / ins_height[:, None] * 2
        t = self.normalized_timestamps[self.cur_frame]
        t = t.unsqueeze(0).repeat(local_means.shape[0], 1)
    ...
    def compute_reg_loss(self):
        ...
        if out_of_bound_losscfg is not None:
            ...
            stop_after = out_of_bound_losscfg.get("stop_after", -1)
            local_xyz_deformed = self._gs_cache["local_xyz_deformed"]
            if w > 0 and local_xyz_deformed is not None and (stop_after < 0 or self.step <= stop_after):
                per_pts_size = self.instances_size[self.point_ids[..., 0]]

                # Only use finite values to prevent this regularizer from producing NaNs.
                valid_mask = torch.isfinite(local_xyz_deformed).all(dim=-1) & torch.isfinite(per_pts_size).all(dim=-1)
                if valid_mask.any():
                    local_valid = local_xyz_deformed[valid_mask]
                    size_valid = per_pts_size[valid_mask]
                    per_point_penalty = torch.relu(local_valid.abs() - size_valid / 2)
                    per_point_penalty = torch.nan_to_num(per_point_penalty, nan=0.0, posinf=1e4, neginf=0.0)
                    loss_dict["out_of_bound_loss"] = per_point_penalty.mean() * w
                else:
                    loss_dict["out_of_bound_loss"] = torch.zeros((), device=self.device)
        return loss_dict
