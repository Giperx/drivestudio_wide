...
def k_nearest_sklearn(x: torch.Tensor, k: int):
    ...
    # Handle edge cases where requested neighbors exceed available samples.
    if n_samples == 0:
        return np.zeros((0, 1), dtype=np.float32), np.zeros((0, 1), dtype=np.float32)
    if n_samples == 1:
        return np.zeros((1, 1), dtype=np.float32), np.full((1, 1), -1, dtype=np.float32)

    k_eff = min(k, n_samples - 1)

    # Build the nearest neighbors model
    nn_model = NearestNeighbors(n_neighbors=k_eff + 1, algorithm="auto", metric="euclidean").fit(x_np)
