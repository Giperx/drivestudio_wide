...
class VanillaGaussians(nn.Module):
    ...
    def get_scaling(self):
        # Clamp log-scales before exp to avoid overflow producing inf/NaN downstream.
        clamped_scales = torch.clamp(self._scales, min=-15.0, max=15.0)
        if self.ball_gaussians:
            if self.gaussian_2d:
                scaling = torch.exp(clamped_scales).repeat(1, 2)
                scaling = torch.cat([scaling, torch.zeros_like(scaling[..., :1])], dim=-1)
                return scaling
            else:
                return torch.exp(clamped_scales).repeat(1, 3)
        else:
            if self.gaussian_2d:
                scaling = torch.exp(clamped_scales)
                scaling = torch.cat([scaling[..., :2], torch.zeros_like(scaling[..., :1])], dim=-1)
                return scaling
            else:
                return torch.exp(clamped_scales)
    @property
    ...
    def after_train(
        ...
        with torch.no_grad():
            if self.num_points == 0:
                self.filter_mask = torch.zeros(0, dtype=torch.bool, device=self.device)
                self.xys_grad_norm = torch.zeros(0, device=self.device)
                self.vis_counts = torch.zeros(0, device=self.device)
                self.max_2Dsize = torch.zeros(0, device=self.device)
                return

            if radii.numel() == 0 or xys_grad.numel() == 0:
                return

            if not hasattr(self, "filter_mask") or self.filter_mask.numel() != self.num_points:
                self.filter_mask = torch.ones(self.num_points, dtype=torch.bool, device=self.device)
    ...
    def split_gaussians(self, split_mask: torch.Tensor, samps: int) -> Tuple:
        ...
        size_fac = 1.6
        delta_log_scale = math.log(size_fac)
        new_scales = (self._scales[split_mask] - delta_log_scale).repeat(samps, 1)
        self._scales[split_mask] = self._scales[split_mask] - delta_log_scale
    ...
    def get_gaussians(self, cam: dataclass_camera) -> Dict:
        ...
        if self.sh_degree > 0:
            ...
            viewdirs = viewdirs / (viewdirs.norm(dim=-1, keepdim=True) + 1e-6)
    ...
    def compute_reg_loss(self):
        loss_dict = {}
        # Some node classes may be fully culled during training.
        # Skip regularizers when no gaussian points are left to avoid NaN from empty reductions.
        if self._scales.shape[0] == 0:
            return loss_dict
        ...
        if sharp_shape_reg_cfg is not None:
            ...
            if self.step % step_interval == 0:
                # scale regularization in a numerically stable form.
                log_scale = self._scales
                log_ratio = log_scale.amax(dim=-1) - log_scale.amin(dim=-1)
                ratio = torch.exp(torch.clamp(log_ratio, max=20.0))
                ratio = torch.nan_to_num(
                    ratio,
                    nan=float(max_gauss_ratio),
                    posinf=float(max_gauss_ratio) * 10.0,
                    neginf=float(max_gauss_ratio),
                )
                scale_reg = torch.relu(ratio - float(max_gauss_ratio)).mean() * w
