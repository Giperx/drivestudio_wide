...
class DeformableGaussians(VanillaGaussians):
    ...
    def get_gaussians(self, cam: dataclass_camera) -> Dict:
        ...
        if delta_scale is not None:
            activated_scales = torch.exp(torch.clamp(self._scales + delta_scale, min=-15.0, max=15.0))
        else:
            activated_scales = torch.exp(torch.clamp(self._scales, min=-15.0, max=15.0))
