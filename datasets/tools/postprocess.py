...
def match_and_postprocess(
    ...
    def _to_int_frame_keys(track_dict):
        normalized = {}
        for k, v in track_dict.items():
            try:
                ...
                if isinstance(v, dict) and "time" in v:
                    ...
                    frame_id = int(os.path.splitext(os.path.basename(str(k)))[0]) - 1
            normalized[frame_id] = v
        return normalized
    ...
    for cam_id in camera_list:
        ...
        raw_pred_track = _to_int_frame_keys(PredTracksDict[cam_id])
        raw_gt_track = _to_int_frame_keys(GTTracksDict[cam_id])
        frame_ids = sorted(set(raw_pred_track.keys()) | set(raw_gt_track.keys()))
        num_f = len(frame_ids) # number of aligned frames
        
        for fi, frame_id in enumerate(frame_ids):
            pred_track_f = raw_pred_track.get(frame_id, _empty_pred_frame())
        ...
        for fi, frame_id in enumerate(frame_ids):
            gt_track_f = raw_gt_track.get(frame_id, _empty_gt_frame())
