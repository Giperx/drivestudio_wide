...
def cleanup_cuda_memory():
    """Best-effort GPU memory cleanup between camera runs."""
    gc.collect()
...
class HMR2_4dhuman(PHALP):
    ...
    def setup_visualizer(self):
        # In headless preprocessing, skip OpenGL renderer initialization.
        # This avoids eglMakeCurrent failures when visual outputs are not needed.
        if self.cfg.render.enable:
            super().setup_visualizer()
        else:
...
def initialize_config(config_name="config"):
    # Re-entrant safety for batch processing across many scenes.
...
def run_4DHumans(
    ...
    for cam_id in camera_list:
        cam_results_path = os.path.join(output_dir, f'cam_{cam_id}.pkl')
        if os.path.exists(cam_results_path):
            logger.info(f"Results for camera {cam_id} already exists at {cam_results_path}")
            continue
        
        cfg = initialize_config()
        # PHALP's renderer depends on EGL/OpenGL context. Enable it only when
        # visualization is explicitly requested.
        cfg.render.enable = bool(verbose)
        ...
        try:
            ...
            demo_pkls = glob(os.path.join(results_dir, 'demo_*.pkl'))
            if not demo_pkls:
                raise FileNotFoundError(
                    f"PHALP did not produce any demo_*.pkl under {results_dir} for camera {cam_id}."
                )

            # PHALP may emit different demo indices across runs. Pick the newest output.
            latest_demo_pkl = max(demo_pkls, key=os.path.getmtime)
            os.replace(latest_demo_pkl, cam_results_path)
        finally:
            if phalp_tracker is not None:
                del phalp_tracker
            cleanup_cuda_memory()
            # Always clear Hydra state so next camera/scene can re-initialize safely.
            if GlobalHydra.instance().is_initialized():
                GlobalHydra.instance().clear()
    ...
    for cam_id in camera_list:
        cam_results_path = os.path.join(output_dir, f'cam_{cam_id}.pkl')
        pred_tracks_allcam[cam_id] = joblib.load(cam_results_path)
