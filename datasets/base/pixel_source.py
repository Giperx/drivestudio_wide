...
class ScenePixelSource(abc.ABC):
    ...
    def prepare_novel_view_render_data(self, dataset_type: str, traj: torch.Tensor) -> list:
        ...
        if dataset_type == "argoverse":
            cam_id = 1 if 1 in self.camera_data else list(self.camera_data.keys())[0]  # Use cam_id 1 for Argoverse dataset if available
        else:
            cam_id = list(self.camera_data.keys())[0]  # Use first available cam_id for other datasets
