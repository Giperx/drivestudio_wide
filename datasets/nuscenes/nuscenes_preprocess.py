...
class NuScenesProcessor(object):
    ...
    def convert_one(self, scene_idx):
        ...
        if "cam2ego_extrinsics" in self.process_keys:
            keyframe_timestamps = np.array(self.get_keyframe_timestamps(scene_data), dtype=np.int64)
            self.save_cam2ego_extrinsics(scene_data, scene_idx, keyframe_timestamps)
            print(f"Processed cam2ego extrinsics for scene {str(scene_idx).zfill(3)}")
        ...
        if "ego_pose" in self.process_keys:
            keyframe_timestamps = np.array(self.get_keyframe_timestamps(scene_data), dtype=np.int64)
            self.save_ego_pose_interpolated(scene_data, scene_idx, keyframe_timestamps)
            print(f"Processed ego poses for scene {str(scene_idx).zfill(3)}")
        if "depth_map" in self.process_keys:
            keyframe_timestamps = np.array(self.get_keyframe_timestamps(scene_data), dtype=np.int64)
            self.save_depth_map_interpolated(scene_data, scene_idx, keyframe_timestamps)
            print(f"Processed depth maps for scene {str(scene_idx).zfill(3)}")
    ...
    def convert_one_interpolated(self, scene_idx):
        ...
        if "calib" in self.process_keys:
            ...
            print(f"Processed calib for scene {str(scene_idx).zfill(3)}")
        if "cam2ego_extrinsics" in self.process_keys:
            self.save_cam2ego_extrinsics(scene_data, scene_idx, interpolated_timestamps)
            print(f"Processed cam2ego extrinsics for scene {str(scene_idx).zfill(3)}")
        if "lidar" in self.process_keys:
            self.save_lidar_interpolated(scene_data, scene_idx, interpolated_timestamps)
            print(f"Processed lidar for scene {str(scene_idx).zfill(3)}")
        if "ego_pose" in self.process_keys:
            self.save_ego_pose_interpolated(scene_data, scene_idx, interpolated_timestamps)
            print(f"Processed ego poses for scene {str(scene_idx).zfill(3)}")
        if "depth_map" in self.process_keys:
            self.save_depth_map_interpolated(scene_data, scene_idx, interpolated_timestamps)
            print(f"Processed depth maps for scene {str(scene_idx).zfill(3)}")
    ...
    def save_cam2ego_extrinsics(self, scene_data, scene_idx, timestamps: np.array):
        """Save per-camera cam-to-ego extrinsics once per scene."""
        for cam_idx, cam_name in enumerate(self.cam_list):
            cam_token = self.find_closest_img_tokens(scene_data, [int(timestamps[0])], cam_name)[0]
    ...
    def save_ego_pose_interpolated(self, scene_data, scene_idx, timestamps: np.array):
        """Save per-frame ego-to-world pose matrices."""
        front_cam = self.cam_list[0]
        closest_tokens = self.find_closest_img_tokens(scene_data, timestamps, front_cam)
    ...
    def save_depth_map_interpolated(self, scene_data, scene_idx, timestamps: np.array):
        """Save per-frame camera depth maps from LiDAR to compressed npz files."""
        closest_lidar_tokens = self.find_cloest_lidar_tokens(scene_data, timestamps)
        closest_cam_tokens = {
            cam_name: self.find_closest_img_tokens(scene_data, timestamps, cam_name)
            for cam_name in self.cam_list
        }

        for frame_idx, lidar_token in enumerate(closest_lidar_tokens):
            lidar_data = self.nusc.get('sample_data', lidar_token)
            lidar_path = os.path.join(self.nusc.dataroot, lidar_data['filename'])
            pc = LidarPointCloud.from_file(lidar_path)
            lidar_points = pc.points[:3, :].T
            ...
            for cam_idx, cam_name in enumerate(self.cam_list):
                cam_token = closest_cam_tokens[cam_name][frame_idx]
                cam_data = self.nusc.get('sample_data', cam_token)
                ...
                cam_lidar_points = (lidar_to_sensor @ lidar_points_homo.T).T
