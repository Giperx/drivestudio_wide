...
def front_center_interp(
    ...
    cam_ids = list(per_cam_poses.keys())
    assert len(cam_ids) > 0, "At least one camera is required for front_center_interp"
    cam0 = cam_ids[0]
    key_poses = per_cam_poses[cam0][::original_frames//4]  # Select every 4th frame as key frame
...
def s_curve(
    ...
    """Create an S-shaped trajectory using the first three cameras."""
    cam_ids = list(per_cam_poses.keys())
    assert len(cam_ids) >= 3, "At least three cameras are required for s_curve"
    cam0, cam1, cam2 = cam_ids[0], cam_ids[1], cam_ids[2]
    key_poses = torch.cat([
        per_cam_poses[cam0][0:1],
        per_cam_poses[cam1][original_frames//4:original_frames//4+1],
        per_cam_poses[cam0][original_frames//2:original_frames//2+1],
        per_cam_poses[cam2][3*original_frames//4:3*original_frames//4+1],
        per_cam_poses[cam0][-1:]
    ], dim=0)
    return interpolate_poses(key_poses, target_frames)
...
def three_key_poses_trajectory(
    ...
    """
    cam_ids = list(per_cam_poses.keys())
    assert len(cam_ids) > 0, "At least one camera is required"
    cam0 = cam_ids[0]

    # First key pose: First frame of front camera
    start_pose = per_cam_poses[cam0][0]
    key_poses = [start_pose]

    # Select camera for middle frame
    middle_frame = int(original_frames // 2)
    chosen_cam = np.random.choice(cam_ids[1:3]) if len(cam_ids) >= 2 else cam0
    ...
    key_poses.append(per_cam_poses[cam0][-1])
